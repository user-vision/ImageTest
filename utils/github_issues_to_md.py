...
def get_csv(repository, parent, auth): 
	REPO = repository  # format is username/repo
	csvfile = 'accessibility_issues.csv'
	csvout = csv.writer(open(csvfile, 'w'))
	csvout.writerow(('id', 'Title', 'Body', 'Created At', 'Updated At'))
	url = 'https://api.github.com/repos/{}/issues?labels={}'.format(repository,parent)
	# print(parent)
	r = requests.get(url, auth=auth)
...
if args.wcag:
	...
	for project in args.projects:
		...
		count = count + 1
		get_csv(repository, parent,auth)
