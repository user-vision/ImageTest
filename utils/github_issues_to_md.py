            ...
            if 'closed' not in issue['state']:
                ...
                if any(project in s['name'] for s in issue['labels']):
                    ...
                    for label in issue['labels']:
                            ...
                            elif re.match(r'High', label['name']) is not None:
                                severity.append(label['name']) 
                            elif re.match(r'Medium', label['name']) is not None:
                                severity.append(label['name']) 
                            elif re.match(r'Low', label['name']) is not None:
                                severity.append(label['name']) 
                            elif re.match(r'Positive', label['name']) is not None:
                                severity.append(label['name']) 
                            elif re.match(r'Observation', label['name']) is not None:
                                severity.append(label['name']) 
                    ...
                    if location:
                        ...
                        for label in location:
                            ...
                            if get_description(url):
                                # descurl = "**[" + get_description(url) + "](" + get_description(url) + ")**"
                                descurl = ""
                            else:
                                descurl = ""
                    ...
                    if re.match(r'^#[ ]', paddedBody) is not None:
                            ...
                            rBody = re.sub(r"\!\[([-a-zA-Z0-9()@:%_\+.~#?'`&\/=_, ]*)\]\(([\w\W]+?)\)", '```{r, fig.cap="' + r"\1" + '", fig.alt="' + r"\1" + '", fig.align="left", out.width="90%", echo=FALSE}\ninclude_image("' + r"\2" + '", img_dir = "images")\n```', rBody)
                    ...
                    rBody = re.sub(r"\!\[([-a-zA-Z0-9()@:%_\+.~#?&\/=_,; '`‘’]*)\]\(([\w\W]+?)\)", '```{r, fig.cap="' + r"\1" + '", fig.alt="' + r"\1" + '", fig.align="left" , echo=FALSE}\ninclude_image("' + r"\2" + '", img_dir = "images")\n```', rBody)
...
def get_issue(issue):
    ...
    for label in issue['labels']:
        if re.match(r'[0-9]*\.', label['name']) is not None: 
            ...
            platform.append(label['name'])
        elif re.match(r'iPadOS', label['name']) is not None:
            platform.append(label['name'])
        elif re.match(r'iPhone', label['name']) is not None:
            platform.append(label['name'])
        elif re.match(r'Android', label['name']) is not None:
...
def get_issues(name, project, state, count):
    ...
    ratings = ["Positive", "High", "Medium", "Low", "Observation"]
    platforms = ["Desktop", "iPhone", "Tablet", "iOS",  "IE", "iPad", "Android", "Firefox", "Chrome", "Safari", "Automated", "Automated findings"]
...
# ##########################################################################################################################
# ##########################################################################################################################
# ################################ For testing against 2.2: ################################################################
# # Delete previous block 
# # Uncomment this block (select text and press 'Ctrl'+'/' )
# ##########################################################################################################################
# ##########################################################################################################################
...
#     donut = '\n(ref:wcag'+ str(count) +') WCAG 2.2 Compliance - '+ str(round(aPass/33*100)) +'% Level A, '+ str(round(aaPass/57*100)) +'% Level AA   \n\n(ref:wcag'+ str(count+1) +') WCAG 2.2 Compliance - '+ str(round(aPass/33*100)) +'% Level A, '+ str(round(aaPass/57*100)) +'% Level AA   \n\n```{r,  fig.cap="(ref:wcag'+ str(count) +')",  fig.alt="(ref:wcag'+ str(count+1) +')", fig.align="left", echo=FALSE}\n\nlibrary(ggplot2)\nlibrary(gridExtra)\nlibrary(grid)\nblank_theme <- theme_minimal()+\n  theme(\n    axis.title.x = element_blank(),\n    axis.title.y = element_blank(),\n    panel.border = element_blank(),\n    panel.grid=element_blank(),\n    axis.ticks = element_blank(),\n    plot.title=element_text(size=16, face="bold"),\n  )\ndat = data.frame(count=c('+str(aPass)+', '+str(aFail)+'), category=c("Pass", "Fail"))\ndat$fraction = dat$count / sum(dat$count)\ndat$ymax = cumsum(dat$fraction)\ndat$ymin = c(0, head(dat$ymax, n=-1))\ndat$category <- factor(dat$category, levels = c("Pass", "Fail"))\np1 = ggplot(dat, aes(fill=category, ymax=ymax, ymin=ymin, xmax=4, xmin=3)) +\n  geom_rect(color="white") +\n  coord_polar(theta="y") +\n  xlim(c(2, 4))\nedu<-p1 + scale_fill_brewer("Level A",palette = "Dark2") + blank_theme + \n  theme(axis.text.x=element_blank()) + theme(legend.position=c(.5, .5)) + ggtitle("") +\n  theme(panel.grid=element_blank()) +\n  theme(axis.text=element_blank()) +\n  theme(axis.ticks=element_blank()) +\n  theme(legend.title = element_text(size=16, face="bold")) +\n  theme(legend.text = element_text(size = 14, face = "bold"))\nedu +  geom_label(aes(label=paste(round(fraction*100, digits = 0),"%\n",category), x=3.5,y=(ymin+ymax)/2), inherit.aes = TRUE, show.legend = FALSE,label.size = 0, color="white") -> p3\ndat = data.frame(count=c('+str(aaPass)+', '+str(aaFail)+'), category=c("Pass", "Fail"))\ndat$fraction = dat$count / sum(dat$count)\ndat$ymax = cumsum(dat$fraction)\ndat$ymin = c(0, head(dat$ymax, n=-1))\ndat$category <- factor(dat$category, levels = c("Pass", "Fail"))\np2 = ggplot(dat, aes(fill=category, ymax=ymax, ymin=ymin, xmax=4, xmin=3)) +\n  geom_rect(color="white") +\n  coord_polar(theta="y") +\n  xlim(c(2, 4))\nedu<-p2 + scale_fill_brewer("Level AA",palette = "Dark2") + blank_theme + \n  theme(axis.text.x=element_blank()) + theme(legend.position=c(.5, .5)) + ggtitle("") +\n  theme(panel.grid=element_blank()) +\n  theme(axis.text=element_blank()) +\n  theme(axis.ticks=element_blank()) +\n  theme(legend.title = element_text(size=16, face="bold")) +\n  theme(legend.text = element_text(size = 14, face = "bold"))\nedu +  geom_label(aes(label=paste(round(fraction*100, digits = 0),"%\n",category), x=3.5,y=(ymin+ymax)/2), inherit.aes = TRUE, show.legend = FALSE,label.size = 0, color="white") -> p4\ngrid.arrange(p3, p4, ncol = 2)\n```\n\n'
...
#         # print(Counter(mylist).keys())
#         # print(Counter(mylist).values())
#         csvout.writelines('### WCAG 2.2 Compliance {-}\n')
